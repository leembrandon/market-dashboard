    ...
    <div className={s.page}>
      ...
      {!loading&&!error&&data&&(
        <div style={{animation:"fadeIn 0.3s ease-out"}}>
          ...
          {recent.length>0&&<div className={s.section}><h3 className={s.sectionTitle}>Last {recent.length} Sessions</h3><div className={s.streakRow}>{recent.map((r,i)=><div key={i} className={s.streakCard}><div className={s.streakDate}>{r.date?.slice(5)||"—"}</div><div className={s.streakDir} style={{color:r.direction==="up"?"var(--green)":"var(--red)"}}>{r.direction==="up"?"▲ up":"▼ down"}</div><div className={s.streakResult}>{r.filled?<span style={{color:"var(--green)"}}>✓ filled</span>:<span style={{color:"var(--red)"}}>✗ unfilled</span>}</div><div className={s.streakMove}>{r.gap_pct!=null?`${Number(r.gap_pct).toFixed(2)}%`:"—"}</div></div>)}</div></div>}
