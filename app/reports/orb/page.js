    ...
    <div className={s.page}>
      ...
      {!loading&&!error&&data&&(
        <div style={{animation:"fadeIn 0.3s ease-out"}}>
          ...
          {/* ── Education accordion — sits between hero and action cards ── */}
          <EducationAccordion content={EDUCATION.orb} />
          ...
          {recent.length>0&&<div className={s.section}><h3 className={s.sectionTitle}>Last {recent.length} Sessions</h3><div className={s.streakRow}>{recent.map((r,i)=>(<Link key={i} href={`/session?symbol=${symbol}&session=${session}&date=${r.date}`} className={s.streakLink}><div className={s.streakCard}><div className={s.streakDate}>{r.date?.slice(5)||"—"}</div><div className={s.streakDir} style={{color:r.first_break==="above"?"var(--green)":"var(--red)"}}>{r.first_break==="above"?"▲":"▼"} {r.first_break||"—"}</div><div className={s.streakResult}>{r.held?<span style={{color:"var(--green)"}}>✓ held</span>:r.trapped?<span style={{color:"var(--red)"}}>✗ trapped</span>:<span style={{color:"var(--amber)"}}>✗ failed</span>}</div><div className={s.streakMove}>{r.move!=null?`${r.move>0?"+":""}${r.move.toFixed(2)}%`:"—"}</div></div></Link>))}</div></div>}
