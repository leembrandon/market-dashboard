...
const REPORT_CONFIG = [
  {
    key: "orb", title: "Opening Range Breakout", href: "/reports/orb", icon: "⧖",
    question: "Should I trade the first breakout or wait?",
    heroStat: (d) => pct(d?.bias?.first_break_reliability), heroLabel: "first break reliability",
    subStats: (d) => [
  ...
  {
    key: "gap_fill", title: "Gap Fill", href: "/reports/gap-fill", icon: "⇅",
    question: "Should I fade this gap or ride it?",
    heroStat: (d) => pct(d?.bias?.fill_rate), heroLabel: "gap fill rate",
    subStats: (d) => [
  ...
  {
    key: "ib", title: "Initial Balance", href: "/reports/ib", icon: "⬌",
    question: "Is today a trend day or a chop day?",
    heroStat: (d) => pct((d?.bias?.ext_above_rate || 0) + (d?.bias?.ext_below_rate || 0)), heroLabel: "extension rate",
    subStats: (d) => [
  ...
  {
    key: "prior_day_levels", title: "Prior Day Levels", href: "/reports/prior-day-levels", icon: "☰",
    question: "Will this level hold or break?",
    heroStat: (d) => pct(d?.bias?.test_rate), heroLabel: "PDH test rate",
    subStats: (d) => [
...
export default function HomePage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchOverview(symbol, 90, session)
      .then(setData)
      .catch(() => setData({}))
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.tickerBar}>
        ...
        <span style={{ width: 16 }} />
        {SESSIONS.map((ses) => (
          <button key={ses.value} onClick={() => setSession(ses.value)}
            className={`${s.tickerBtn} ${session === ses.value ? s.tickerBtnActive : ""}`}>{ses.label}</button>
        ))}
      </div>
            ...
            <Link key={report.key} href={report.href} className={s.cardLink}>
              <div className={s.card}>
                ...
                <div className={s.cardHero}>
                  {loading ? <div className={s.cardHeroLoading}>--</div>
                    : hasData ? <div className={s.cardHeroNumber}>{report.heroStat(rd)}</div>
                ...
                {hasData && !loading && (
                  <>
                    ...
                    <div className={s.cardMeta}>{rd.sample_size} sessions · 90d lookback</div>
                  </>
                )}
              </div>
            </Link>
