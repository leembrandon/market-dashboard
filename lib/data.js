...
export async function fetchSession(symbol, tradeDate, sessionType) {
  const query = [
    "select=*",
    `symbol=eq.${symbol}`,
  ...
  const rows = await supabaseFetch("sessions", query);
  if (!rows || rows.length === 0) return null;
  const row = rows[0];
  if (typeof row.orb_data === "string") row.orb_data = JSON.parse(row.orb_data);
  return row;
}

export async function fetchSessionList(symbol, sessionType, limit = 20) {
  const query = [
    "select=*",
    `symbol=eq.${symbol}`,
    `session_type=eq.${sessionType}`,
